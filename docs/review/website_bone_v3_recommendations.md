...
**Deploy state (UPDATED 2026-07-23 — now DONE):** all **12 article pages are now deployed bone-v3 and live** (verified `#F5F4EF`), carrying the HITL copy + the `/research/`-404 footer fix + breadcrumb + typo fixes. WP IDs were discovered (all Pages under parent 230, except root-level 888/850) and wired into `deploy.py`'s registry; deployed with `--force` to bypass the voice gate for the pre-existing prose debt (Will-authorized). Backups saved under `docs/review/pre-deploy-backup-*.json`. Remaining: (a) the **voice cleanup** on the original prose so they rejoin the validated (non-`--force`) deploy path; (b) the **root-level dupes** (`/what-is-an-arr-waterfall/` = page 873, `/crpo-vs-arr/`, `/semrush-adobe-acquisition-case-study/` = 888) still need 301s → `/resources/` canonical; (c) `778`/NRR still not published (404); (d) the **Jan-2026 batch AEO/SEO rework** — see `docs/review/jan2026-batch-aeo-seo-plan.md`.
